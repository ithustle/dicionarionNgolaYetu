...
require_once '../controller/controllerConsoantes.php';
require_once '../controller/controllerVerbos.php';
require_once '../controller/controllerVogais.php';
require_once '../classes/Kikongo.php';
require_once '../classes/Umbundu.php';
require_once '../classes/Linguas.php';

$kik = "Não disponível";
$umb = "Não disponível";

$portugues = new Linguas();
$kikongo = new Kikongo();
$umbundo = new Umbundu();

$kikongo->setLingua($_GET['pesquisa']);
$umbundo->setLingua($_GET['pesquisa']);
$portugues->setLingua($_GET['pesquisa']);
$portuga = NULL;

foreach ($kikongo->traduzirKikongo() as $tradK){
    
    if (isset($tradK['kikongo'])) {
        $kik = $tradK['kikongo'];
    }
}

foreach ($umbundo->traduzirUmbundu() as $tradU){
    
    if (isset($tradU['Umbundo'])) {
        $umb = $tradU['Umbundo'];
    }
}

foreach ($portugues->linguaPortuguesa() as $port){
    
        $portuga = $port['portgues'];
}
